...
export default function EditBimbel() {
  const router = useRouter();
  const { id } = router.query;
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [bimbelLoading, setBimbelLoading] = useState(true);
  ...
  useEffect(() => {
    // Ambil data bimbel berdasarkan ID
    const fetchBimbel = async () => {
      if (!id) return;
      
      setBimbelLoading(true);
      try {
        const result = await callApi('get', `${process.env.NEXT_PUBLIC_BIMBEL_API_URL}/bimbel/bimbels/${id}/`);
        
        if (result.success) {
          // Pastikan bimbel ini milik admin yang sedang login
          const userData = JSON.parse(localStorage.getItem('user'));
          if (result.data.admin !== userData.id) {
          ...
          setFormData({
            ...
            kuota_awal: result.data.kuota_awal,
            sisa_kuota: result.data.sisa_kuota,
            deskripsi: result.data.deskripsi || '',
            harga: result.data.harga,
          });
        }
      } catch (error) {
        console.error("Error fetching bimbel:", error);
        router.push('/admin-dashboard');
      } finally {
        setBimbelLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await callApi('put', `${process.env.NEXT_PUBLIC_BIMBEL_API_URL}/bimbel/bimbels/${id}/`, formData);
      
      if (result.success) {
        router.push('/admin-dashboard');
      }
    } catch (error) {
      console.error("Error updating bimbel:", error);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <Head>
        <title>Edit Bimbel - BimbelKu</title>
      </Head>
      ...
      <main className="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-4 py-5 sm:p-6">
            <h1 className="text-lg font-medium leading-6 text-gray-900 mb-4">Edit Bimbel</h1>
